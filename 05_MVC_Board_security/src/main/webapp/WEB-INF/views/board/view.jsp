...
pageEncoding="UTF-8"%>
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions"%> <!-- 사진 다운로시 필요! -->
  ...
  <body>
    <div class="container">
      ...
      <input type="hidden" name="no" value="${vo.no}">
        ...
        <!-- 두가지 방법이 있당 -->
        <!--  a 태그의 속성중에 download로 해도된당 <a href="/board/download?filename=${fn:replace(vo.url, '/upload/', '')}"><img src="${vo.url}"/></a>  --> 
        ...
        <!-- 
        	...
        	* authorize : 권한부여와 관련된것
        	* authentication : 인증과 관련된 것, 계정 정보
        -->
       <%-- 로그인정보를 가지고 알아서 맞춰줌. <sec:authentication property="principal" var="info"/>
         <c: if test="${vo.writer eq info.username}"> 
          <!-- 확실한 권한과 인증을 같이 걸어줘야함. --%>
