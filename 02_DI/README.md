...
- 스프링에서 관리하는 객체를 `빈(Bean)`이라고 하고, 해당 빈들을 관리하는 의미로 컨테이너를 `빈 팩토리(Bean Factory)`라고 한다.
...
```xml
<beans>
    <bean id="studnet" class="com.kh.model.vo.Student"/>
</beans>
```
...
- 생성자를 통해 의존 관계가 있는 빈을 주입하려면 `<contructor-arg>` 요소를 사용한다. (단, 일치하는 매개변수가 있는 생성자가 있어야 된다.)
